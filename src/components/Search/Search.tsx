...
const Search: FC<Props> = ({
  ...
  setSearchQuery,
    ...
    //Route para navegacion
  const router = useRouter();

  // ----------------- HANDLERS ------------------------------
  // asigna los valores para filtrar la busqueda segun el option permitido en el select
  const handleRoomTypeChange = (event: ChangeEvent<HTMLSelectElement>) => {
    setRoomTypeFilter(event.target.value);
  };

  // asigna los valores para filtrar la busqueda segun el input
  const handleSearchQueryChange = (event: ChangeEvent<HTMLInputElement>) => {
    setSearchQuery(event.target.value);
  };

  // Realiza la busqueda en el query segun los valores establecidos anteriormente
